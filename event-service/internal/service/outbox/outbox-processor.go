...
func NewOutboxProcessor(outboxRepo *outboxrepo.OutboxRepo, brokerConn *rabbitmq.PublisherConn) *OutboxProcessor {
...
func (p *OutboxProcessor) ProcessOutbox(ctx context.Context) error {
	...
	for _, record := range stashed {
		...
		err := p.publisherConn.Publish(pubCtx, record.Payload, os.Getenv("RABBITMQ_EVENT_TO_LECTURE_QUEUE"), true)
		cancel()
